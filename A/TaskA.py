...
# Example function using scikit-learn SVC with C=1 and kernel='linear'
def train_and_evaluate_linear_svm(train_features, y_train, 
                                  val_features, y_val,
    ...
    # Validation score
    val_accuracy = svm_linear.score(val_features, y_val)
    print("Linear SVM (C=1) Validation Accuracy:", val_accuracy)

    # Test score
    test_accuracy = svm_linear.score(test_features, y_test)
    print("Linear SVM (C=1) Test Accuracy:", test_accuracy)
...
# ---------------------------
# This part is use for finding the best solution for SVM
# ---------------------------
# param_grid = {
#     'C': [0.1, 1, 10],
...
#pfrint(f"SVM (Best)      - Val Acc: {val_acc:.4f},   Test Acc: {test_acc:.4f}")

# Print Best Random Forest Hyperparameters
print("\nBest Random Forest Hyperparameters:")
for param_name in sorted(grid_search_rf.best_params_):
    print(f"{param_name}: {grid_search_rf.best_params_[param_name]}")

cv_results_df = pd.DataFrame(grid_search_rf.cv_results_)
relevant_columns = ['param_n_estimators', 'param_max_depth', 'param_min_samples_split',
                    'mean_test_score', 'std_test_score', 'rank_test_score']
print("\nGrid Search Results:")
print(cv_results_df[relevant_columns].sort_values('rank_test_score'))
...
#  Train a RandomForest on just 2D data
